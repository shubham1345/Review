...
namespace DAL.Repository
{
    public class DapperRepo : IDapper
    {
        ...
        protected async Task<IEnumerable<T>> QueryAsync<T>(string query, object? parameters = null, CommandType commandType = CommandType.StoredProcedure)
        {
            IDbConnection connection = new SqlConnection(_config.GetConnectionString(Connectionstring));
            return await connection.QueryAsync<T>(query, parameters, commandType: commandType);
        }

        protected async Task<T> QuerySingleOrDefaultAsync<T>(string query, object? parameters = null, CommandType commandType = CommandType.StoredProcedure)
        {
            IDbConnection connection = new SqlConnection(_config.GetConnectionString(Connectionstring));
